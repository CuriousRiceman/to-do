...
export function generateProjectTasks(parentContainer, forWhichProject) {
    ...
    Object.keys(projectTasks).forEach((taskTitle) => {
        ...
        modifyElement.addEventListener("click", () => {
            ...
            // Adds the taskTitle to the class list so it can be retrieved upon submitting the task to make changes to localStorage
            const submitTaskButton = document.querySelector('.submit-task-button');
            submitTaskButton.classList.add(taskDetails.whichProject);
            submitTaskButton.dataset.TaskName = taskTitle;
        });
